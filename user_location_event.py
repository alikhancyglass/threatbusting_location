...
MAP_NORMAL_LOACATION_INDEX = "rawappidaccmgt_v1"
TARGET_INDEX = "user_location_test100"
...
def scan_anomaly_locations(escl, index):
    '''
    Takes ESCL intsance
    Return list of docs(type: dict) of anomaly loactions and list of unique users_ids created anomaly location events.
    '''
    anomaly_docs = []
    user_ids = set()
    ...
    for doc in scan(client=escl, index=index, query=search_param):
        ...
        anomaly_docs.append(target_doc_baseline)
...
def main():
    ...
    anomaly_docs, user_ids = scan_anomaly_locations(src_escl, MAP_ML_EVENT_INDEX_)
    # DST Site Data Upload
    ...
    if not dst_escl.indices.exists(index=TARGET_INDEX):
        ...
        create_index(dst_escl, set_index=TARGET_INDEX, set_schema=user_location_schema)
        print('Cretaed index: ', TARGET_INDEX, 'in ', DST_SITE)
    else:
        print(TARGET_INDEX, 'is already in', DST_SITE, 'no need to create index')
    # IF I re run this command it writes duplicates, is there any duplicate prevention     
